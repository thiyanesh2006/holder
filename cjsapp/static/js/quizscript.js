...
function displayQuiz(mode) {
    const quizTitle = document.getElementById('quizTitle');
    const quizContent = document.getElementById('quizContent');
    
    quizTitle.textContent = `${mode.charAt(0).toUpperCase() + mode.slice(1)} `;
    quizContent.innerHTML = '';

    quizData[mode].forEach((question, index) => {
...
function submitQuiz() {
    ...
    alert(`You have completed your quiz with ${score} out of ${questions.length} congrates!`);
